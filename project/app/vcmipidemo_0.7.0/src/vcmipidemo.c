...
***
*** @author  Copyright (c) 2018-2022 Vision Components.
...
#define  DEMO_VERSION        (  7)  /**<       Version: -.X.-   */
...
int  main(int argc, char *argv[])
{
	...
	// Set up configuration and apply command line parameters if set.
	{
		...
		rc =  change_options_by_commandline(argc, argv, &optShutter, &optGain, &optMaxCaptures, &optFBOutIff1, acFramebufferDev, &optStdOutIff1, &optFileOutIff1, &optBufCount, &optVideoDevId, &optWidth, &optHeight, &optX0, &optY0, &optFps, &cfgWB);
...
int  process_capture(unsigned int pixelformat, char *st, int dx, int dy, int pitch, int stdOutIff1, int netSrvOutIff1, VCImgNetCfg *imgnetCfg, int fbOutIff1, int fileOutIff1, int frameNr, char *pcFramebufferDev, VCWhiteBalCfg *cfgWB)
{
	...
	if(1==fbOutIff1)
	{
		...
		if(rc<0){ee=-21+100*rc; goto fail;}
	}
	...
	while((opt =  getopt(argc, argv, "g:s:i:fabp:od:r:w:")) != -1)
	{
		switch(opt)
		{
			default:
				...
				printf("  -p,  Frames per seconds to request.                                          \n");
			...
			case 'p':  *fps        = atol(optarg);  printf("Setting FPS Request to %d.\n",*fps);        break;
