...
MODULE_PARM_DESC(
	sensor_mode,
	"VC Sensor Mode: IMX327,412: 0=10bit_stream; IMX290,412: 0=10bit_stream, 1=10bit_4lanes; IMX296,297: 0=10bit_stream 1=10bit_ext_trig 2=mode0+binning 3=mode1+binning");
...
#define IMX900_GAIN_MAX 0xee
#define IMX900_GAIN_STEP 1
#define IMX900_GAIN_DEFAULT 0x00
...
#define MIPI_FREQ_54M 54000000
#define IMX900_8BIT_PIXEL_RATE (MIPI_FREQ_54M * 2 / 8 * 2)
...
static int imx_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	int ret = 0;
	dev_info(&client->dev, "SET STREAM interval: %s\n",
		 enable ? "ON" : "OFF");
	...
	if (!enable) {
...
#if 1
			ret = vc_mipi_common_trigmode_write(
				priv->rom, 0, 0, 0, 0, 0, &priv->tpf,
				&priv->rom_table); /* disable external trigger counter */
			if (ret)
				dev_err(&client->dev,
					"Error %d disabling trigger counter\n",
					ret);
			ret = vc_mipi_common_reg_write_table(client,
							     priv->trait->stop);
			if (ret < 0)
				dev_err(&client->dev,
					"STOP Error %d setting registers\n",
					ret);
			return ret;
			break;
#endif
		...
		case IMX415_MODEL_COLOR:
			...
			if (ret < 0)
				dev_err(&client->dev,
					"REINIT: Error %d setting registers\n",
					ret);
			...
			ret = vc_mipi_common_rom_init(
				client, priv->rom, priv->cur_mode->sensor_mode);
			if (ret)
				return ret;
			...
			if (ret)
				dev_err(&client->dev,
					"REINIT: Error %d disabling trigger counter\n",
					ret);

			ret = v4l2_ctrl_handler_setup(&priv->ctrl_handler);
			if (ret < 0)
				dev_err(&client->dev,
					"REINIT: Error %d setting controls\n",
					ret);
	...
	case IMX296_MODEL_COLOR_S:
		...
		if (priv->cur_mode->sensor_mode <
		    (IMX296_S_SENSOR_MODE_MAX / 2)) {
			...
			if (sen_reg(priv, H_START_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_START_LOW),
					(priv->crop_rect.left / 2) & 0xff);
			if (sen_reg(priv, H_START_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_START_HIGH),
					(priv->crop_rect.left / 2) >> 8);
			if (sen_reg(priv, H_OUTPUT_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_OUTPUT_LOW),
					(priv->crop_rect.width / 2) & 0xff);
			if (sen_reg(priv, H_OUTPUT_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_OUTPUT_HIGH),
					(priv->crop_rect.width / 2) >> 8);
			// use value for sensor registers!
			if (sen_reg(priv, V_START_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_START_LOW),
					(priv->crop_rect.top) & 0xff);
			if (sen_reg(priv, V_START_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_START_HIGH),
					(priv->crop_rect.top) >> 8);
			if (sen_reg(priv, V_OUTPUT_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_OUTPUT_LOW),
					(priv->crop_rect.height) & 0xff);
			if (sen_reg(priv, V_OUTPUT_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_OUTPUT_HIGH),
					(priv->crop_rect.height) >> 8);
#ifdef PLATFORM_QCOM
			ret |= vc_mipi_common_reg_write(
				client, 0x4182,
				(priv->crop_rect.height) & 0xff);
			ret |= vc_mipi_common_reg_write(
				client, 0x4183, (priv->crop_rect.height) >> 8);
#endif
		} else {
			dev_info(&client->dev, "IMX296S: [SENSORMODE] %d\n",
				 priv->cur_mode->sensor_mode);
			...
			if (sen_reg(priv, H_START_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_START_LOW),
					(priv->crop_rect.left) & 0xff);
			if (sen_reg(priv, H_START_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_START_HIGH),
					(priv->crop_rect.left) >> 8);
			if (sen_reg(priv, H_OUTPUT_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_OUTPUT_LOW),
					(priv->crop_rect.width) & 0xff);
			if (sen_reg(priv, H_OUTPUT_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, H_OUTPUT_HIGH),
					(priv->crop_rect.width) >> 8);
			// half value for sensor registers!
			if (sen_reg(priv, V_START_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_START_LOW),
					(priv->crop_rect.top / 2) & 0xff);
			if (sen_reg(priv, V_START_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_START_HIGH),
					(priv->crop_rect.top / 2) >> 8);
			if (sen_reg(priv, V_OUTPUT_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_OUTPUT_LOW),
					(priv->crop_rect.height / 2) & 0xff);
			if (sen_reg(priv, V_OUTPUT_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_OUTPUT_HIGH),
					(priv->crop_rect.height / 2) >> 8);
#ifdef PLATFORM_QCOM
			ret |= vc_mipi_common_reg_write(
				client, 0x4182,
				(priv->crop_rect.height / 2) & 0xff);
	...
	case IMX335_MODEL_COLOR:
		...
		if (sen_reg(priv, H_START_LOW))
			ret |= vc_mipi_common_reg_write(
				client, sen_reg(priv, H_START_LOW),
				(priv->crop_rect.left + 76) & 0xff);
		if (sen_reg(priv, V_START_LOW))
			ret |= vc_mipi_common_reg_write(
				client, sen_reg(priv, V_START_LOW),
				(priv->crop_rect.top + 176) & 0xff);
		//FALLSTHROUGH
	...
	case OV9281_MODEL_COLOR:
		if (priv->cur_mode->sensor_ext_trig) {
			...
			ret |= vc_mipi_common_reg_write_table(
				client, priv->trait->stop);

			if (!ret) {
				ret |= vc_mipi_common_trigmode_write(
					priv->rom,
					priv->cur_mode->sensor_ext_trig,
					priv->exposure_time, priv->io_config,
					priv->enable_extrig, priv->sen_clk,
					&priv->tpf, &priv->rom_table);
				/*
					 * ! CAUTION !
					 * for correct trigger timing the trigger is NOW enabled after DMA start!
					 * */
				return (ret | vc_mipi_common_reg_write(
						      priv->rom, 0x0108,
						      priv->enable_extrig));
			} else
				dev_err(&client->dev,
					"OV9281 - ERROR !!! - unable to set sensor trigmode\n");
		}
		break;
#ifdef PLATFORM_QCOM
	case IMX296_MODEL_MONOCHROME:
	...
	case IMX297_MODEL_MONOCHROME:
	case IMX297_MODEL_COLOR:
		ret |= vc_mipi_common_reg_write(client, 0x4182,
						priv->crop_rect.height & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x4183,
						priv->crop_rect.height >> 8);
		break;
	...
	case IMX296_MODEL_COLOR_S:
		...
		if (priv->cur_mode->sensor_mode >=
			...
			if (sen_reg(priv, V_SIZE_LOW))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_SIZE_LOW),
					(priv->crop_rect.height / 2) & 0xff);
			if (sen_reg(priv, V_SIZE_HIGH))
				ret |= vc_mipi_common_reg_write(
					client, sen_reg(priv, V_SIZE_HIGH),
					(priv->crop_rect.height / 2) >> 8);
			break;
		}
		//FALLTHROUGH
	...
	ret = vc_mipi_common_trigmode_write(
		priv->rom, priv->cur_mode->sensor_ext_trig, priv->exposure_time,
		priv->io_config, priv->enable_extrig, priv->sen_clk, &priv->tpf,
		&priv->rom_table);
	if (ret)
		return ret;
	 ...
	 * for correct trigger timing the trigger is NOW enabled after DMA start!
	 * */
	return (ret | vc_mipi_common_reg_write(priv->rom, 0x0108,
					       priv->enable_extrig));
}
				...
				struct v4l2_subdev_frame_interval *ival)
{
	...
	dev_info(&client->dev, "GET Frame interval: %u/%u s = %d us\n",
		 ...
		 ival->interval.numerator * 1000 /
			 (ival->interval.denominator / 1000));
...
static int imx_set_frame_length(struct imx *priv, u32 val, u32 vmax_min)
{
	...
	err = vc_mipi_common_write_mbreg(client, (u16)(base + of + 0),
					 frame_length & 0x07ffff, 3);
	if (err)
		goto fail;
...

	{
		...
		int vmax = 0;
		// VMAX
		reg = vc_mipi_common_read_mbreg(client, (u16)(base + of + 0),
						&vmax, 3);
				...
				struct v4l2_subdev_frame_interval *ival)
{
	...
	div_max = priv->trait->supported_modes[priv->cur_mode->sensor_mode]
			  .max_fps;
	...
	dev_info(&client->dev,
		 "INITSET Frame interval: %u/%u s = %d us div_max = %d\n",
		 tpf->numerator, tpf->denominator,
		 tpf->numerator * 1000 / (tpf->denominator / 1000), div_max);
	...
	*tpf = priv->tpf;
	...
	if (ret < 0)
		dev_err(&client->dev,
			"SET Frame interval : Error %d setting controls\n",
			ret);
...
static int imx_exposure_290_327(struct imx *priv)
{
	...
	if (priv->cur_mode->sensor_ext_trig)
		return vc_mipi_common_trigmode_write(
			priv->rom, priv->cur_mode->sensor_ext_trig,
			priv->exposure_time, priv->io_config,
			priv->enable_extrig, priv->sen_clk, &priv->tpf,
			&priv->rom_table);
	...
	if (priv->exposure_time < 38716) {
		// range 1..1123
		exposure = (1124 * 20000 -
			    (u32)(priv->exposure_time) * 29 * 20 * lf) /
			   20000;
		...
		ret |= vc_mipi_common_reg_write(client, 0x3022,
						(exposure >> 16) & 0x01);
		ret |= vc_mipi_common_reg_write(client, 0x3021,
						(exposure >> 8) & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x3020,
						exposure & 0xff);
	} else // range 1123..
	{
		exposure = (1 * 20000 +
		...
		ret |= vc_mipi_common_reg_write(client, 0x301A,
						(exposure >> 16) & 0x01);
		ret |= vc_mipi_common_reg_write(client, 0x3019,
						(exposure >> 8) & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x3018,
						exposure & 0xff);
	}
...
static int imx_exposure_462(struct imx *priv)
{
	...
	if (((1 * 20000 + (u32)(priv->exposure_time) * 66 * 20 * lf) / 20000) >
	    1123) {
		// range 1123..
		exposure = (1 * 20000 +
			    (u32)(priv->exposure_time) * 66 * 20 * lf) /
			   20000;
		...
		if ((exposure > 0) && (exposure < 1025)) {
			...
			ret |= vc_mipi_common_reg_write(
				client, 0x3022, (exposure >> 16) & 0x01);
			ret |= vc_mipi_common_reg_write(client, 0x3021,
							(exposure >> 8) & 0xff);
			ret |= vc_mipi_common_reg_write(client, 0x3020,
							exposure & 0xff);
...
static int imx_exposure_412(struct imx *priv)
{
	...
	if (priv->exposure_time < 92694) {
		exposure = (((u32)(priv->exposure_time) * 20000 / 16) -
			    (1932 * 20000 / 5792)) /
			   20000;
		dev_info(&client->dev, "EXP = %d \n", exposure);

		ret |= vc_mipi_common_reg_write(client, 0x0202,
						(exposure >> 8) & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x0203,
						exposure & 0xff);
	}
...
static int imx_exposure_415(struct imx *priv)
{
	...
	if (priv->exposure_time <= (2250 - 8) * 4 * lf) {
		exposure = (2250 - ((u32)(priv->exposure_time) * 20000) /
					   (4 * lf * 20000));
		dev_info(&client->dev, "EXP = %d \n", exposure);

		ret |= vc_mipi_common_reg_write(client, 0x3052,
						(exposure >> 16) & 0x07);
		ret |= vc_mipi_common_reg_write(client, 0x3051,
						(exposure >> 8) & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x3050,
						exposure & 0xff);
		...
		ret |= vc_mipi_common_reg_write(client, 0x3026,
						(exposure >> 16) & 0x07);
		ret |= vc_mipi_common_reg_write(client, 0x3025,
						(exposure >> 8) & 0xff);
		ret |= vc_mipi_common_reg_write(client, 0x3024,
						exposure & 0xff);
	}
...
#define OV9281_DIGITAL_EXPOSURE_MAX ((((876 * 9100) + 9099) / 9100) << 4)
	...
	flash = (((u32)(((priv->exposure_time) * 1000) + 9099) /
		  (9100))); // calculate in ns
	flash_mod = (((u32)(((priv->exposure_time) * 1000) + 9099) %
		      (9100))); // calculate in ns
	if (flash_mod < 4050)
		flash = flash - 1;
...
static int imx_exposure_imx(struct imx *priv, s32 expMin0, s32 expMin1,
			    s32 expMax, s32 nrLines, s32 tOffset, s32 h1Period,
			    s32 vMax, u32 shs_min, u32 vmax_min)
{
	struct i2c_client *client = v4l2_get_subdevdata(&priv->subdev);
	int ret = 0;
	u32 exposure = 0;
...
#if 1
	{
		...
		{
			vmax = 0;
			reg = vc_mipi_common_read_mbreg(
				client, (u16)(base + offv), &vmax, 3);
	...
	if ((exposure <= vMax)) {
		...
		shs = (vMax - ((u32)(priv->exposure_time) * 16384 - tOffset) /
				      h1Period);
		if (shs < shs_min)
			shs = shs_min;
		...
		divisor = h1Period;
		remainder = (u32)(do_div(
			divresult,
			divisor)); // caution: division result value at dividend!
	...
	if ((regaddr = sen_reg(priv, EXPOSURE_LOW)) &&
	    (sen_reg(priv, EXPOSURE_MIDDLE)) &&
	    (sen_reg(priv, EXPOSURE_HIGH))) {
		ret = vc_mipi_common_write_mbreg(client, regaddr,
						 shs & 0x07ffff, 3);
		dev_info(&client->dev, "SHS 24bit\n");
	} else if ((regaddr = sen_reg(priv, EXPOSURE_LOW)) &&
		   (sen_reg(priv, EXPOSURE_MIDDLE))) {
		ret = vc_mipi_common_write_mbreg(client, regaddr, shs & 0xffff,
						 2);
...
#define imx_exposure_178(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_178, EXPOSURE_TIME_MIN2_178,  \
			 EXPOSURE_TIME_MAX_178, NRLINES_178, TOFFSET_178,      \
			 H1PERIOD_178, VMAX_178, SHS_MIN_178, VMAX_MIN_178)
...
#define imx_exposure_183(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_183, EXPOSURE_TIME_MIN2_183,  \
			 EXPOSURE_TIME_MAX_183, NRLINES_183, TOFFSET_183,      \
			 H1PERIOD_183, VMAX_183, SHS_MIN_183, VMAX_MIN_183)
...
#define imx_exposure_226(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_226, EXPOSURE_TIME_MIN2_226,  \
			 EXPOSURE_TIME_MAX_226, NRLINES_226, TOFFSET_226,      \
			 H1PERIOD_226, VMAX_226, SHS_MIN_226, VMAX_MIN_226)
...
#define imx_exposure_250(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_250, EXPOSURE_TIME_MIN2_250,  \
			 EXPOSURE_TIME_MAX_250, NRLINES_250, TOFFSET_250,      \
			 H1PERIOD_250, VMAX_250, SHS_MIN_250, VMAX_MIN_250)

#define imx_exposure_252(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_250, EXPOSURE_TIME_MIN2_250,  \
			 EXPOSURE_TIME_MAX_250, NRLINES_250, TOFFSET_250,      \
			 H1PERIOD_250, VMAX_250, SHS_MIN_250, VMAX_MIN_250)

#define imx_exposure_264(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_250, EXPOSURE_TIME_MIN2_250,  \
			 EXPOSURE_TIME_MAX_250, NRLINES_250, TOFFSET_250,      \
			 H1PERIOD_250, VMAX_250, SHS_MIN_250, VMAX_MIN_250)

#define imx_exposure_265(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_250, EXPOSURE_TIME_MIN2_250,  \
			 EXPOSURE_TIME_MAX_250, NRLINES_250, TOFFSET_250,      \
			 H1PERIOD_250, VMAX_250, SHS_MIN_250, VMAX_MIN_250)
...
#define imx_exposure_566(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_566, EXPOSURE_TIME_MIN2_566,  \
			 EXPOSURE_TIME_MAX_566, NRLINES_566, TOFFSET_566,      \
			 H1PERIOD_566, VMAX_566, SHS_MIN_566, VMAX_MIN_566)
...
#define imx_exposure_568(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_568, EXPOSURE_TIME_MIN2_568,  \
			 EXPOSURE_TIME_MAX_568, NRLINES_568, TOFFSET_568,      \
			 H1PERIOD_568, VMAX_568, SHS_MIN_568, VMAX_MIN_568)
...
#define EXPOSURE_TIME_MIN2_585                                                 \
	((VMAX_585 - SHS_MIN_585) * (H1PERIOD_585 / 16384))
#define EXPOSURE_TIME_MAX_585 2000000

#define imx_exposure_585(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_585, EXPOSURE_TIME_MIN2_585,  \
			 EXPOSURE_TIME_MAX_585, NRLINES_585, TOFFSET_585,      \
			 H1PERIOD_585, VMAX_585, SHS_MIN_585, VMAX_MIN_585)
...
#define imx_exposure_900(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_900, EXPOSURE_TIME_MIN2_900,  \
			 EXPOSURE_TIME_MAX_900, NRLINES_900, TOFFSET_900,      \
			 H1PERIOD_900, VMAX_900, SHS_MIN_900, VMAX_MIN_900)
...
#define imx_exposure_392(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_392, EXPOSURE_TIME_MIN2_392,  \
			 EXPOSURE_TIME_MAX_392, NRLINES_392, TOFFSET_392,      \
			 H1PERIOD_392, VMAX_392, SHS_MIN_392, VMAX_MIN_392)
...
#define imx_exposure_273(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_273, EXPOSURE_TIME_MIN2_273,  \
			 EXPOSURE_TIME_MAX_273, NRLINES_273, TOFFSET_273,      \
			 H1PERIOD_273, VMAX_273, SHS_MIN_273, VMAX_MIN_273)
...
#define imx_exposure_296(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_296, EXPOSURE_TIME_MIN2_296,  \
			 EXPOSURE_TIME_MAX_296, NRLINES_296, TOFFSET_296,      \
			 H1PERIOD_296, VMAX_296, SHS_MIN_296, VMAX_MIN_296)
...
#define imx_exposure_297(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_297, EXPOSURE_TIME_MIN2_297,  \
			 EXPOSURE_TIME_MAX_297, NRLINES_297, TOFFSET_297,      \
			 H1PERIOD_297, VMAX_297, SHS_MIN_297, VMAX_MIN_297)
...
#define imx_exposure_335(priv)                                                 \
	imx_exposure_imx(priv, EXPOSURE_TIME_MIN_335, EXPOSURE_TIME_MIN2_335,  \
			 EXPOSURE_TIME_MAX_335, NRLINES_335, TOFFSET_335,      \
			 H1PERIOD_335, VMAX_335, SHS_MIN_335, VMAX_MIN_335)
...
static int imx_gain(struct imx *priv)
{
	...
	default:
		...
		if (sen_reg(priv, GAIN_LOW))
			ret |= vc_mipi_common_reg_write(
				client, sen_reg(priv, GAIN_LOW),
				priv->digital_gain & 0xff);
	}
	return ret;
}
static void imx_get_module_inf(struct imx *imx, struct rkmodule_inf *inf)
{
	memset(inf, 0, sizeof(*inf));
	strscpy(inf->base.sensor, str_imx_model(imx->model),
		sizeof(inf->base.sensor));
	strscpy(inf->base.module, str_imx_model(imx->model),
		sizeof(inf->base.module));
	strscpy(inf->base.lens, "None", sizeof(inf->base.lens));
}
static int imx_get_channel_info(struct imx *imx,
				struct rkmodule_channel_info *ch_info,
				struct i2c_client *client)
{
	u32 vc[PAD_MAX];

	dev_info(&client->dev, "imx_get_channel_info index %d\n",
		 ch_info->index);
	...
	if (ch_info->index < PAD0 || ch_info->index >= PAD_MAX)
		return -EINVAL;
	if (*imx->cur_datalanes == 4) {
		vc[PAD0] = V4L2_MBUS_CSI2_CHANNEL_2;
		vc[PAD1] = V4L2_MBUS_CSI2_CHANNEL_1;
		vc[PAD2] = V4L2_MBUS_CSI2_CHANNEL_0;
		vc[PAD3] = V4L2_MBUS_CSI2_CHANNEL_2;
	} else {
		vc[PAD0] = V4L2_MBUS_CSI2_CHANNEL_0;
	}
	ch_info->vc = vc[ch_info->index];

	ch_info->width = imx->cur_mode->width;
	ch_info->height = imx->cur_mode->height;

	switch (imx->cur_mode->sensor_depth) {
	case 8:
		ch_info->bus_fmt = mbus_pixelcode_by_model_depth8(imx->model);
		break;
	case 10:
		ch_info->bus_fmt = mbus_pixelcode_by_model_depth10(imx->model);
		break;
	case 12:
		ch_info->bus_fmt = mbus_pixelcode_by_model_depth12(imx->model);
		break;
	case 14:
		ch_info->bus_fmt = mbus_pixelcode_by_model_depth14(imx->model);
		break;
	default:
		return -EINVAL;
	}
		       ...
		       struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		fmt->reserved[0] =
			V4L2_MBUS_CSI2_CHANNEL_0; //TODO for other lane configs
	}
...
__imx_get_pad_format(struct imx *imx, struct v4l2_subdev_pad_config *cfg,
...
__imx_get_pad_crop(struct imx *imx, struct v4l2_subdev_pad_config *cfg,
		   unsigned int pad, enum v4l2_subdev_format_whence which)
{
	pr_info("imx_get_pad_crop\n");
	switch (which) {
	case V4L2_SUBDEV_FORMAT_TRY:
		v4l2_info(&imx->subdev, "V4L2_SUBDEV_FORMAT_TRY");
		return v4l2_subdev_get_try_crop(&imx->subdev, cfg, pad);
	case V4L2_SUBDEV_FORMAT_ACTIVE:
		v4l2_info(&imx->subdev, "crop_rect (l,t,w,h): %d %d %u %u\n",
			  imx->crop_rect.left, imx->crop_rect.top,
			  imx->crop_rect.width, imx->crop_rect.height);
			     ...
			     struct v4l2_subdev_selection *sel)
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		sel->r.left = CROP_START(imx->cur_mode->width, DST_WIDTH);
		sel->r.width = DST_WIDTH;
		sel->r.top = CROP_START(imx->cur_mode->height, DST_HEIGHT);
		sel->r.height = DST_HEIGHT;
		return 0;
	}
	return -EINVAL;
	sel->r = imx->crop_rect;
	// I want to log the crop_rect to a file
...
static int IMX900_g_mbus_config(struct v4l2_subdev *subdev, unsigned int pad_id,
				struct v4l2_mbus_config *config)
{
	struct i2c_client *client = v4l2_get_subdevdata(subdev);
	struct imx *imx = to_imx(client);
	const struct imx_mode *mode = imx->cur_mode;
	u32 val = 0;
	u32 lane_num = *imx->cur_datalanes;
...
static long IMX900_ioctl(struct v4l2_subdev *subdev, unsigned int cmd,
			 void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(subdev);
	struct imx *imx = to_imx(client);
	...
	case PREISP_CMD_SET_HDRAE_EXP:
		//TODO
		// if (imx->cur_mode->hdr_mode == HDR_X2)
		// 	ret = IMX900_set_hdrae(IMX900, arg);
		// else if (imx->cur_mode->hdr_mode == HDR_X3)
		// 	ret = IMX900_set_hdrae_3frame(IMX900, arg);
		break;
	case RKMODULE_GET_MODULE_INFO:
		imx_get_module_inf(imx, (struct rkmodule_inf *)arg);
		break;
	case RKMODULE_GET_HDR_CFG:
		hdr = (struct rkmodule_hdr_cfg *)arg;
		hdr->esp.mode = HDR_NORMAL_VC;
		hdr->hdr_mode = imx->cur_mode->hdr_mode;
		break;
	case RKMODULE_SET_HDR_CFG:
		//TODO
		// hdr = (struct rkmodule_hdr_cfg *)arg;
		// w = imx->cur_mode->width;
		// h = imx->cur_mode->height;
		// for (i = 0; i < imx->cfg_num; i++) {
		// 	if (w == imx->support_modes[i].width &&
		// 	    h == imx->support_modes[i].height &&
		// 	    imx->support_modes[i].hdr_mode == hdr->hdr_mode) {
		// 		imx->cur_mode = &imx->support_modes[i];
		// 		break;
		// 	}
		// }
		// if (i == imx->cfg_num) {
		// 	dev_err(&imx->client->dev,
		// 		"not find hdr mode:%d %dx%d config\n",
		// 		hdr->hdr_mode, w, h);
		// 	ret = -EINVAL;
		// } else {
		// 	w = imx->cur_mode->hts_def - imx->cur_mode->width;
		// 	h = imx->cur_mode->vts_def - imx->cur_mode->height;
		// 	__v4l2_ctrl_modify_range(imx->hblank, w, w, 1, w);
		// 	__v4l2_ctrl_modify_range(imx->vblank, h,
		// 		IMX900_VTS_MAX - imx->cur_mode->height,
		// 		1, h);
		// 	imx->cur_vts = imx->cur_mode->vts_def;
		// 	pixel_rate = (u32)link_freq_menu_items[imx->cur_mode->mipi_freq_idx] / imx->cur_mode->bpp * 2 *
		// 		     imx->bus_cfg.bus.mipi_csi2.num_data_lanes;
		// 	__v4l2_ctrl_s_ctrl_int64(imx->pixel_rate,
		// 				 pixel_rate);
		// 	__v4l2_ctrl_s_ctrl(imx->link_freq,
		// 			   imx->cur_mode->mipi_freq_idx);
		// }
		break;
	case RKMODULE_SET_CONVERSION_GAIN:
		//TODO
		// ret = IMX900_set_conversion_gain(imx, (u32 *)arg);
		break;
	case RKMODULE_SET_QUICK_STREAM:
		v4l2_info(&imx->subdev, "RKMODULE_SET_QUICK_STREAM\n");

		imx_s_stream(&imx->subdev, *((u32 *)arg));
		...
		// if (stream)
		// 	ret = IMX900_write_reg(imx->client, IMX900_REG_CTRL_MODE,
		// 		IMX900_REG_VALUE_08BIT, IMX900_MODE_STREAMING);
		// else
		// 	ret = IMX900_write_reg(imx->client, IMX900_REG_CTRL_MODE,
		// 		IMX900_REG_VALUE_08BIT, IMX900_MODE_SW_STANDBY);

		break;
	case RKMODULE_GET_CHANNEL_INFO:
		ch_info = (struct rkmodule_channel_info *)arg;
		ret = imx_get_channel_info(imx, ch_info, client);
		break;
	case RKMODULE_GET_SYNC_MODE:
		sync_mode = (u32 *)arg;
		*sync_mode = imx->sync_mode;
		break;
	case RKMODULE_SET_SYNC_MODE:
		sync_mode = (u32 *)arg;
		imx->sync_mode = *sync_mode;
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
			     ...
			     struct v4l2_subdev_selection *sel)
{
	...
	rect.left = clamp(ALIGN(sel->r.left, 2), (s32)priv->content.hStart,
			  (s32)priv->content.hOutSize - 1);
	rect.top = clamp(ALIGN(sel->r.top, 2), (s32)priv->content.vStart,
			 (s32)priv->content.vOutSize - 1);
	rect.width = clamp_t(unsigned int, ALIGN(sel->r.width, 2), 1,
			     priv->content.hOutSize - rect.left);
	rect.height = clamp_t(unsigned int, ALIGN(sel->r.height, 2), 1,
			      priv->content.vOutSize - rect.top);
	...
	if (rect.width != __crop->width || rect.height != __crop->height) {
		 ...
		 */
		__format =
			__imx_get_pad_format(priv, cfg, sel->pad, sel->which);
...
#ifdef CONFIG_VIDEO_V4L2_SUBDEV_API
static int imx_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct imx *priv = to_imx(client);
	...
	try_fmt->code = priv->cur_pixelcode;
...
static int imx_video_probe(struct i2c_client *client, struct imx *priv)
{
	...
	if (sen_reg(priv, MODEL_ID_HIGH))
		ret = vc_mipi_common_reg_read(client,
					      sen_reg(priv, MODEL_ID_HIGH));
	if (ret < 0) {
	...
	if (!((model_id == check_id) || (model_id == check_id2) ||
	      (model_id == check_id3))) {
	...
	if (sen_reg(priv, CHIP_REV)) // chip_id register not always availabale
	{
		...
		if (ret < 0) {
			dev_err(&client->dev,
				"Failure to read Chip ID (low byte)\n");
			goto done;
		}
		chip_id = ret;
	}
...
static int imx_ctrls_init(struct v4l2_subdev *sd)
{
	...
	h_blank = 0x05dc * 2 - priv->cur_mode->width;

	vblank_def = 0x672 - priv->cur_mode->height; //TODO

	// IMX900->cur_vts = mode->vts_def;

	pixel_rate = (u32)MIPI_FREQ_54M / 8 * 2 * 2;
	...
	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_HFLIP, 0,
			  1, 1, 0);

	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_VFLIP, 0,
			  1, 1, 0);

	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops,
			  V4L2_CID_ANALOGUE_GAIN, priv->trait->gain_digital_min,
			  priv->trait->gain_digital_max, 1,
			  priv->trait->gain_digital_default);
	v4l2_info(sd, "V4L2_CID_ANALOGUE_GAIN added");

	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_GAIN,
			  priv->trait->gain_digital_min,
			  priv->trait->gain_digital_max, 1,
			  priv->trait->gain_digital_default);

	priv->ctrl_hblank = v4l2_ctrl_new_std(&priv->ctrl_handler,
					      &imx_ctrl_ops, V4L2_CID_HBLANK,
					      h_blank, h_blank, 1, h_blank);
	v4l2_info(sd, "V4L2_CID_HBLANK added");

	if (priv->ctrl_hblank)
		priv->ctrl_hblank->flags |= V4L2_CTRL_FLAG_READ_ONLY;
	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_VBLANK,
			  vblank_def, IMX900_VTS_MAX - priv->cur_mode->height,
			  1, vblank_def);

	v4l2_info(sd, "V4L2_CID_VBLANK added");
...
#define IMX296_PIXEL_ARRAY_HEIGHT 1088
	...
	priv->vblank =
		v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops,
				  V4L2_CID_VBLANK, 30,
				  1048575 - IMX296_PIXEL_ARRAY_HEIGHT, 1, 30);
#endif
#endif

	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_EXPOSURE,
			  priv->trait->exposure_min, priv->trait->exposure_max,
			  1, priv->trait->exposure_default);
...
#if 1
	{
		...
		priv->ctrl_pixelrate = v4l2_ctrl_new_std(
			&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_PIXEL_RATE,
			1, (priv->cur_linkfreq / 2), 1, (divresult / 2));

		priv->cur_pixelrate = divresult;

		priv->ctrl_linkfreq = v4l2_ctrl_new_int_menu(
			&priv->ctrl_handler, &imx_ctrl_ops, V4L2_CID_LINK_FREQ,
			0, 0, &priv->cur_linkfreq); // currently only index 0
...
static int imx_devicetree_read(struct i2c_client *client,
			       struct imx_dtentry *dtentry)
{
	...
	prop = of_get_property(node, "io-config",
			       &len); // read property from device node
	...
	prop = of_get_property(node, "external-trigger-mode-overwrite",
			       &len); // read property from device node

	if (prop && len == sizeof(u32)) {
		dtentry->enable_extrig = be32_to_cpu(
			prop[0]); // set ext trig source to pin=1 or selftriggered=4
		dev_info(&client->dev,
			 "read property external-trigger-mode-overwrite = %d\n",
			 dtentry->enable_extrig);
	} else {
		prop = of_get_property(node, "external-trigger-mode",
				       &len); // read property from device node
		...
		if (prop && len == sizeof(u32)) {
			dtentry->enable_extrig = be32_to_cpu(
				prop[0]); // set ext trig source to pin=1 or selftriggered=4
			dev_info(&client->dev,
				 "read property external-trigger-mode = %d\n",
				 dtentry->enable_extrig);
		} else
			dtentry->enable_extrig =
				1; // set ext trig source to pin=1 as fallback
	}

	prop = of_get_property(node, "sensor-mode",
			       &len); // read property from device node

	if (prop && len == sizeof(u32)) {
		dtentry->sensor_mode =
			be32_to_cpu(prop[0]); // read property sensor_mode
	...
	if (!node) {
		...
		if (prop && len == sizeof(u32)) {
			dtentry->sen_clk = be32_to_cpu(
				prop[0]); // set sensor oscillator clock in HZ normal=54Mhz IMX183=72Mhz
			dev_info(&client->dev,
				 "read property clock-frequency = %d\n",
				 dtentry->sen_clk);
		}
		of_node_put(node);
	}
	...
	dtentry->data_lanes = fwnode_property_read_u32_array(
		of_fwnode_handle(endpoint), "data-lanes", NULL, 0);
	if (dtentry->data_lanes) // number of entries in data-lanes property
		dev_info(&client->dev, "read property data-lanes = %d\n",
			 dtentry->data_lanes);

	len = fwnode_property_read_u64_array(of_fwnode_handle(endpoint),
					     "link-frequencies",
					     &(dtentry->linkfreq), 1);
	if (!len) // first link-frequency in property array found
		dev_info(&client->dev,
			 "read property link-frequencies[0] = %lld\n",
			 dtentry->linkfreq);
	...
	if (ret) {
		dtentry->sync_mode = NO_SYNC_MODE;
		dev_err(&client->dev, "could not get sync mode!\n");
	} else {
		if (strcmp(sync_mode_name, RKMODULE_EXTERNAL_MASTER_MODE) == 0)
			dtentry->sync_mode = EXTERNAL_MASTER_MODE;
		else if (strcmp(sync_mode_name,
				RKMODULE_INTERNAL_MASTER_MODE) == 0)
			dtentry->sync_mode = INTERNAL_MASTER_MODE;
		else if (strcmp(sync_mode_name, RKMODULE_SLAVE_MODE) == 0)
			dtentry->sync_mode = SLAVE_MODE;
	}
...
static int
imx_get_model_and_mode(struct i2c_client *client, struct imx_dtentry *dtentry,
		       struct vc_rom_table *rom_table, int color,
		       enum imx_model *model, const struct imx_trait **trait,
		       const struct imx_mode **cur_mode, s32 *cur_pixelcode,
		       s64 *cur_linkfreq, u8 **cur_datalanes, int stereo)
{
	*model = imx_model_by_mod_id(rom_table->mod_id, color, stereo);

	if (IMX_MODEL_UNKNOWN == *model) {
		dev_err(&client->dev,
			"VC Sensor model: Unknown Sensor Module ID %d\n",
			rom_table->mod_id);
		return -EINVAL;
	}
	...
	*trait = imx_trait_by_model(*model);
	...
	if (NULL == *trait) {
		dev_err(&client->dev,
			"VC Sensor device-tree:  No traits of sensor model %s (%d)\n",
			str_imx_model(*model), *model);
		return -EINVAL;
	}
	...
	if ((dtentry->sensor_mode >= (rom_table->nr_modes)) ||
		...
		dev_err(&client->dev,
			"VC Sensor device-tree:  sensor-mode %d must be smaller than %d and non-negative\n",
			dtentry->sensor_mode, (*trait)->sensor_mode_max);
		return -EINVAL;
	}
	...
	*cur_linkfreq =
		rom_table->mode[dtentry->sensor_mode].format_mipi->data_rate;
	...
	if (*cur_linkfreq != dtentry->linkfreq) {
		dev_err(&client->dev,
			"VC Sensor device-tree:  Link frequency of rom table %lld differs from device tree %lld entry\n",
			*cur_linkfreq, dtentry->linkfreq);
#ifdef PLATFORM_QCOM
	...
	case 8:
		*cur_pixelcode = mbus_pixelcode_by_model_depth8(*model);
		break;
	case 10:
		*cur_pixelcode = mbus_pixelcode_by_model_depth10(*model);
		break;
	case 12:
		*cur_pixelcode = mbus_pixelcode_by_model_depth12(*model);
		break;
	case 14:
		*cur_pixelcode = mbus_pixelcode_by_model_depth14(*model);
		break;
	default:
		return -EINVAL;
	}
	if (*cur_pixelcode < 0) {
		dev_err(&client->dev,
			"VC Sensor device-tree:  Unknown MBus Pixel Code for sensor model %s (%d), bit depth %d\n",
			str_imx_model(*model), *model,
			(*cur_mode)->sensor_depth);
		return -EINVAL;
	}

	/* check consistence */
	*cur_datalanes = &(
		rom_table->mode[dtentry->sensor_mode].format_mipi->data_lanes);
	...
	if (*(*cur_datalanes) != dtentry->data_lanes) {
		dev_err(&client->dev,
			"VC Sensor device-tree:  Number of data-lanes must be %d for sensor-mode %d\n",
	...
	dev_info(
		&client->dev,
		"VC Sensor device-tree has configured %d data-lanes! [ sensor_mode = %d ]\n",
...
static int imx_probe(struct i2c_client *client, const struct i2c_device_id *did)
#else
...
#endif
{
	...
	if ((ret = vc_mipi_common_platform_init(
		     client, &priv->io_regulator, &priv->core_regulator,
		     &priv->analog_regulator, &priv->rst_gpio)))
		goto err_out;
	...
	if ((ret = vc_mipi_common_rom_table_read(client, &priv->rom,
						 &priv->rom_table, &priv->color,
						 &priv->stereo)))
		goto err_out;
	...
	if (priv->color == 1 && priv->rom_table.mod_id == 0x296 &&
	    dtentry.data_lanes == 4) {
		*priv->cur_datalanes = 4;
		priv->color = 2;
		dev_info(&client->dev, "RGB initialized\n");
	}

	if ((ret = imx_get_model_and_mode(
		     client, &dtentry, &priv->rom_table, priv->color,
		     &priv->model, &priv->trait, &priv->cur_mode,
		     &priv->cur_pixelcode, &priv->cur_linkfreq,
		     &priv->cur_datalanes, priv->stereo)))
		goto err_out;

	if ((ret = vc_mipi_common_rom_init(client, priv->rom,
					   priv->cur_mode->sensor_mode)))
		goto err_out;

	if ((ret = vc_mipi_common_read_sen_reg_content(client, &priv->rom_table,
						       &priv->content)))
		goto err_out;
...
#endif
	priv->subdev.internal_ops = &imx415_internal_ops;
...
#else
	ret = v4l2_async_register_subdev_sensor_common(&priv->subdev);
		if (ret) {
...
static const struct i2c_device_id imx_id[] = { { "vc_mipi_modules_0", 0 }, {} };
